...
class ItemCategory extends MY_Controller{
    ...
    public function addDieProcess(){
        $data = $this->input->post();
        $this->data['category_id'] = $data['id'];
        $this->data['processList'] = $this->process->getProcessList();
        $this->data['dieProcessList'] = $this->itemCategory->getDieProcessList(['category_id'=>$data['id']]);
        $this->load->view('item_category/die_process',$this->data);
    }

    public function saveDieProcess(){
        $data = $this->input->post();
        $errorMessage = array();
        if(empty($data['process_id'])){
            $errorMessage['general_error'] = "Process required.";
        }

        if(!empty($errorMessage)):
            $this->printJson(['status'=>0,'message'=>$errorMessage]);
        else:
            $this->printJson($this->itemCategory->saveDieProcess($data));
        endif;
    }
}
?>