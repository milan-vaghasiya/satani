...
class RejectionReview extends MY_Controller
{
    ...
    public function bulkRejection(){
        $this->data['headData']->pageTitle = "Bulk Rejection";
		$this->data['headData']->controller = "rejectionReview";
        $this->data['rejectionComments'] = $this->comment->getCommentList(['type'=>1]);
        $this->load->view("rejection_review/bulk_rejection",$this->data);
    }

    public function getBulkRejectionRows(){
        $data = $this->input->post(); 
    ...
    public function saveBulkRejecReview(){
        $data = $this->input->post(); 
