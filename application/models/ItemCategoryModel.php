...
class ItemCategoryModel extends MasterModel{
    ...
    public function saveDieProcess($data){
        try{
            $this->db->trans_begin();
            ...
            foreach($data['process_id'] AS $key=>$process_id){
                ...
                $result = $this->store("die_process",$processData);
            }
    ...
    public function getDieProcessList($param = []){
        $queryData['tableName'] = 'die_process';
        $queryData['select'] = 'die_process.*';

        if(!empty($param['processDetail'])){
            $queryData['select'] .= ',process_master.process_name';
            $queryData['leftJoin']['process_master'] = 'process_master.id = die_process.process_id';
        }
        if(!empty($param['category_id'])){
            $queryData['where']['die_process.category_id'] = $param['category_id'];
        }
        return $this->rows($queryData);
    }
}
