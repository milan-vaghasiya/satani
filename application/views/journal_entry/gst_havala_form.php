        ...
        <div class="row">
            <div class="col-12">
				<div class="col-12">
                    <div class="card">
                        <div class="card-body">
							<form autocomplete="off" data-res_function="resGstHavalaEntry" id="saveGstHavalaEntry">
								<div class="col-md-12">
											...
											<table id="journalEntryData" class="table table-striped table-borderless" >
												<thead class="thead-dark">
													<tr>
														...
														<th style="width:15%;">Havala Amount</th>
														<th>Remark</th>
													</tr>
												</thead>
                        ...
                        <div class="card-footer bg-facebook">
                            <div class="col-md-12"> 
                                <button type="button" class="btn waves-effect waves-light btn-success float-right save-form" onclick="customStore({'formId':'saveGstHavalaEntry','fnsave':'saveGstHavalaEntry'});" ><i class="fa fa-check"></i> Save </button>
...
function loadData(){
    ...
    var cm_id = $("#cm_id").val();
    var system_code = "'CLIGSTACC','CLIGSTINTACC','CLIGSTPENAACC','CLIGSTFEESACC','CLIGSTOTHACC','CLCGSTACC','CLCGSTINTACC','CLCGSTPENAACC','CLCGSTFEESACC','CLCGSTOTHACC','CLSGSTACC','CLSGSTINTACC','CLSGSTPENAACC','CLSGSTFEESACC','CLSGSTOTHACC','CLGSTCESSACC','CLGSTCESSINTACC','CLGSTCESSPENAACC','CLGSTCESSFEESACC','CLGSTCESSOTHACC'";
...
function AddRow(data) {
	...
	cell = $(row.insertCell(-1));
	cell.html(data.org_cl_balance);
	cell.append(crDrInput);

	var priceInput = $("<input/>", { type: "text", name: "itemData["+itemCount+"][price]", class:'form-control floatOnly',value: data.price, style:"width:65%;" });
    var havalaCrDr = $("<input/>", { type: "text", class:'form-control',value: data.cl_balance_type, readonly:true, style:"width:35%;"});
	cell = $(row.insertCell(-1));
	cell.html('<div class="input-group" id="havalaAmount'+itemCount+'"></div>');
    $("#havalaAmount"+itemCount).append(priceInput);
    $("#havalaAmount"+itemCount).append(havalaCrDr);
...
function resGstHavalaEntry(data,formId){
    if(data.status==1){
        $('#'+formId)[0].reset();
