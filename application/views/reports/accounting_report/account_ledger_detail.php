...
<?php if(in_array($ledgerData->group_code,["SD","SC"])): ?>
    dataTblBtns = {
        dom: {
            button: {
                className: "btn btn-outline-dark"
        ...
        buttons:[ 
            ...
            {
                text: 'Bill Wise',
                className : 'bill-wise-btn',
                action: function (){ 
			        window.open(base_url + 'billWise/index/' + encodeURIComponent(window.btoa(JSON.stringify($('#acc_id').val()))));
                } 
            },
            {
                text: 'Refresh',
