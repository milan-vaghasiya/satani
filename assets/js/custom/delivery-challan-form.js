...
function Edit(data, button) {
	...
	setTimeout(function(){
		if(data.stock_eff == 1){
			...
			var batchDetail = {'postData':{'item_id': data.item_id, 'id' : data.id, 'batchDetail': data.batch_detail,'location_ids':location_ids,'qty_readonly':qty_readonly,'with_opt_qty':1},'table_id':"batchDetail",'tbody_id':'batchTrans','tfoot_id':'','fnget':'getBatchWiseItemStock'};
...
function resItemDetail(response = ""){
    if(response != ""){
		...
		var batchDetail = {'postData':{'item_id': itemDetail.id, 'party_id' : $("#party_id").val(), 'id' : '','location_ids': '','with_opt_qty':1},'table_id':"batchDetail",'tbody_id':'batchTrans','tfoot_id':'','fnget':'getBatchWiseItemStock'};
		getTransHtml(batchDetail);
    }else{
